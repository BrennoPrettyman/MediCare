...
document.addEventListener("submit", (event) => {
        ...
        if (allowed == true && samePassword == true){
            ...
            alert("A senha não corresponde com \"a senha anterior\".")
        }
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    senhaInput.addEventListener("focusout", () => {
        var frase = "";
        if (senhaInput.value.length < 7){
            frase = "Quantidade mínima de caracteres não correspondida , Mínimo necessário: "+(7-senhaInput.value.length)+".\n"
        }
        const isContainsNumber = /^(?=.*[0-9]).*$/;
        if (!isContainsNumber.test(senhaInput.value)){
            frase += "Necessário algum caracter númerico.\n"
        }
        const isContainsSymbol = /^(?=.*[~`!@#$%^&*()--+={}\[\]|\\:;"'<>,.?/_₹]).*$/;
        if (!isContainsSymbol.test(senhaInput.value)){
            frase += "Necessário algum caracter especial."
        }
        if (frase.length > 0){
            alert(frase);
        }
